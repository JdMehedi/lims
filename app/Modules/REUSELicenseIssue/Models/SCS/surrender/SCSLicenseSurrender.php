...
namespace App\Modules\REUSELicenseIssue\Models\SCS\surrender;
...
class SCSLicenseSurrender extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;
    protected $table = 'scs_license_surrender';
    private $formPath = 'SCS.Surrender';
    ...
    public function createForm($currentInstance): string
    {
        ...
        return strval( view( "REUSELicenseIssue::SCS.Surrender.form", $data ) );
    }
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('SCS', $this->process_type_id, $processData->id, $this->table, 'SUR', $appData->id);
        }
        ...
        if ( $request->get( 'actionBtn' ) == 'submit' && $processData->status_id == 1 ) {
            ...
            //Send SMS
            $loginControllerInstance->SendSmsService('APP_SUBMIT', ['{$serviceName}' => 'SCS License Surrender', '{$trackingNumber}' => $trackingNumber], $userMobile);
            ...
            $appInfo = [
                ...
                'process_type_name' => 'SCS License Surrender',
                'remarks'           => ''
            ];
        ...
        if ( $processData->status_id == 2 ) {
            ...
            //Send SMS
            $loginControllerInstance->SendSmsService('APP_RESUBMIT', ['{$serviceName}' => 'SCS License Surrender', '{$trackingNumber}' => $trackingNumber], $userMobile);
...
//            $receiverInfo = CommonFunction::getCompanyUsersEmailPhone();
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        // for sub-view
        $shareholders_data = SCSLicenseSurrender::Join( 'shareholders', 'shareholders.app_id', '=', 'scs_license_surrender.id' )
                                                ->where( [
                                                    'scs_license_surrender.id'     => $decodedAppId,
    ...
    public function editForm($processTypeId, $applicationId): JsonResponse
    {
            ...
            $NixRenewData            = SCSLicenseSurrender::find( $applicationId );
            $data['divisions']       = [ '' => 'Select' ] + Area::where( 'area_type', 1 )->orderBy( 'area_nm', 'asc' )->pluck( 'area_nm', 'area_id' )->toArray();
            $data['districts']       = [ '' => 'Select' ] + Area::where( 'area_type', 2 )->orderBy( 'area_nm', 'ASC' )->pluck( 'area_nm', 'area_id' )->toArray();

            $data['appInfo'] = ProcessList::leftJoin( 'scs_license_surrender as apps', 'apps.id', '=', 'process_list.ref_id' )
                ->leftJoin( 'process_status as ps', function ( $join ) use ( $process_type_id ) {
                    $join->on( 'ps.id', '=', 'process_list.status_id' );
                    $join->on( 'ps.process_type_id', '=', DB::raw( $process_type_id ) );
                } )
            ...
            if ( empty( $NixRenewData->issue_tracking_no ) ) {
                $public_html = (string) view( 'REUSELicenseIssue::SCS.Surrender.masterEditV2', $data );
            } else {
                $public_html = (string) view( 'REUSELicenseIssue::SCS.Surrender.edit', $data );
            }
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        if ( ! empty( $data['master_data']->renew_tracking_no ) ) {
            ...
            $data['appInfo']    = ProcessList::leftJoin( 'scs_license_issue as apps', 'apps.id', '=', 'process_list.ref_id' )
                ->leftJoin( "scs_license_master as ms", function ( $join ) {
        ...
        $public_html = (string) view( 'REUSELicenseIssue::SCS.Surrender.search', $data );
