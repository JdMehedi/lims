...
namespace App\Modules\REUSELicenseIssue\Models\MNO\surrender;
...
class MNOLicenseSurrender extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'mno_license_cancellation';
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval(view("REUSELicenseIssue::MNO.Surrender.form", $data));
    ...
    public function storeForm($request, $currentInstance): RedirectResponse
    {
        ...
        if ( empty( $license_no ) ) {
            Session::flash( 'error', "Invalid License No [MNOS-006]" );

            return redirect()->back()->withInput();
        }

        if ( $request->get( 'app_id' ) ) {
            $appData     = MNOLicenseSurrender::find( Encryption::decodeId($request->get('app_id')));
            $processData = ProcessList::where( [
                'process_type_id' => $this->process_type_id,
                'ref_id'          => $appData->id
            ] )->first();
        } else {
            $appData     = new MNOLicenseSurrender();
       ...
       if ($request->hasFile('declaration_q3_images')) {
        ...
        $path = 'uploads/mno-license-surrender/' . $yearMonth;
        if (!file_exists($path)) {
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('MNO', $this->process_type_id, $processData->id, $this->table, 'SUR', $appData->id);
        }
        ...
        if ( $processData->status_id == 2 ) {
            $appInfo = [
                ...
                'process_type_name' => 'MNO License Surrender',
                'remarks'           => '',
            ];
        ...
        return redirect( 'client/mno-license-cancellation/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
    ...
    public function viewForm($processTypeId, $applicationId): JsonResponse
    {
        ...
        $data['process_type_id'] = $process_type_id;
            ...
            $data['appInfo']   = ProcessList::leftJoin( 'mno_license_cancellation as apps', 'apps.id', '=', 'process_list.ref_id' )
        ...
        $public_html = (string) view( 'REUSELicenseIssue::MNO.Surrender.view', $data );
    ...
    public function editForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $public_html = (string) view('REUSELicenseIssue::MNO.Surrender.edit', $data );
    ...
    public function fetchData($request, $currentInstance): JsonResponse{
        ...
        if ( empty( $data['license_no'] ) ) {
            $public_html = (string) view( 'REUSELicenseIssue::MNO.Surrender.search-blank', $data );
        ...
        if ( ($data['master_data']!=null) && $data['master_data']->renew_tracking_no) {
            ...
            ->leftJoin( 'mno_license_master as ms', function ( $join ) use ( $issue_process_type_id ) {
            ...
            ->leftJoin( 'sp_payment as sfp', function ( $join ) use ( $issue_process_type_id ) {
                $join->on( 'sfp.app_id', '=', 'process_list.ref_id' );
                $join->on( 'sfp.process_type_id', '=', DB::raw( 58 ) );       // comes for issue, so have to have payment for issue -> 1
            } )
            ->where( 'ms.license_no', $request->license_no )
        ...
        $public_html = (string) view( 'REUSELicenseIssue::MNO.Surrender.search', $data );
