...
namespace App\Modules\REUSELicenseIssue\Models\MNO\amendment;
...
class MNOLicenseAmendment extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'mno_license_amendment';
    ...
    public function createForm( $currentInstance ): string {
       ...
       return strval(view("REUSELicenseIssue::MNO.Amendment.master", $data));
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        if ( empty( $license_no ) ) {
            Session::flash( 'error', 'Invalid License No [MNOA-006]' );
        ...
        if ( $request->get( 'app_id' ) ) {
            ...
            $appData     = new MNOLicenseAmendment();
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('MNO', $this->process_type_id, $processData->id, $this->table, 'AMM', $appData->id);
        }
        //=================================================payment code==========================
        ...
        if ( $processData->status_id == 2 ) {
            $appInfo = [
                ...
                'process_type_name' => 'MNO License Amendment',
                'remarks'           => '',
            ];
        ...
        if ( $processData->status_id == - 1 ) {
            Session::flash( 'success', 'Successfully updated the MNO Amendment Application!' );
        } elseif ( $processData->status_id == 1 ) {
            Session::flash( 'success', 'Successfully MNO Amendment Application Submitted !' );
        } elseif ( $processData->status_id == 2 ) {
            Session::flash( 'success', 'Successfully MNO Amendment Application Re-Submitted !' );
        } else {
            Session::flash( 'error', 'Failed due to Application Status Conflict. Please try again later! [MNOA-007]' );
        }

        return redirect( '/mno-license-amendment/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['appInfo'] = ProcessList::leftJoin( 'mno_license_amendment as apps', 'apps.id', '=', 'process_list.ref_id' )
        ...
        $public_html = (string) view( 'REUSELicenseIssue::MNO.Amendment.masterView', $data );
    ...
    public function editForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $public_html = (string) view( 'REUSELicenseIssue::MNO.Amendment.masterEdit', $data );
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        $data['divisions'] = [ '' => 'Select' ] + Area::where( 'area_type', 1 )->orderBy( 'area_nm', 'asc' )->pluck( 'area_nm', 'area_id' )->toArray();
        $data['districts'] = [ '' => 'Select' ] + Area::where( 'area_type', 2 )->orderBy( 'area_nm', 'ASC' )->pluck( 'area_nm', 'area_id' )->toArray();
        $data['master_data'] = MNOLicenseMaster::where('license_no', $request->license_no)->first();
        ...
        if (($data['master_data']!=null) && $data['master_data']->renew_tracking_no) {
            $data['appInfo'] = ProcessList::leftJoin( 'mno_license_renew as apps', 'apps.id', '=', 'process_list.ref_id' )
                                      ...
                                      ->leftJoin( 'process_status as ps', function ( $join ) use ( $process_type_id ) {
                                          $join->on( 'ps.id', '=', 'process_list.status_id' );
                                          $join->on( 'ps.process_type_id', '=', DB::raw( 59 ) );
                                      } )
                                      ->leftJoin( 'sp_payment as sfp', function ( $join ) use ( $process_type_id ) {
                                          $join->on( 'sfp.app_id', '=', 'process_list.ref_id' );
                                          $join->on( 'sfp.process_type_id', '=', DB::raw(  59 ) );       // comes for renew, so have to have payment for issue -> 1
                                      } )
                                      ->where( 'ms.license_no', $request->license_no )
                                      ->where('process_list.process_type_id', 59)
            ...
            $data['appInfo'] = ProcessList::leftJoin( 'mno_license_issue as apps', 'apps.id', '=', 'process_list.ref_id' )
                                      ...
                                      ->where( 'ms.license_no', $request->license_no )
                                      ->where('process_list.process_type_id', 58)
                                      ->where( 'ms.status', 1 )                         // approved status can be renew
                                      ->first( [
                'process_list.id as process_list_id',
                'process_list.desk_id',
        ...
        $public_html = (string) view( 'REUSELicenseIssue::MNO.Amendment.search', $data );
...
//        dd($SpPaymentAmountConfData);
    ...
    private function storeLicenseData( $LicenseAmendmentObj, $request ) {
        ...
        if ( $request->hasFile( 'list_of_clients' ) ) {
            ...
            $path      = 'uploads/mnp-license-ammendment/' . $yearMonth;
            if ( ! file_exists( $path ) ) {
        ...
        $LicenseAmendmentObj->created_at          = Carbon::now();
