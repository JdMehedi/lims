...
namespace App\Modules\REUSELicenseIssue\Models\MNO\renew;
...
use App\Modules\REUSELicenseIssue\Models\MNO\renew\MNOLicenseRenewEquipmentList;
use App\Modules\REUSELicenseIssue\Models\MNO\renew\MNOLicenseRenewTariffChart;
use App\Modules\MNOLicenseIssue\Models\MNOLicenseTariffChart;
...
use App\Modules\REUSELicenseIssue\Models\MNO\issue\MNOLicenseEquipmentList;
...
class MNOLicenseRenew extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'mno_license_renew';
    protected $guarded = [ 'id' ];
    protected $process_type_id;
    protected $mno_issue_process_type_id = 58;
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval( view( 'REUSELicenseIssue::MNO.Renew.form', $data ) );
    }

    public function storeForm( $request, $currentInstance ): RedirectResponse {

        $this->process_type_id = $currentInstance->process_type_id;
//        dd( 'store', $this->process_type_id);

        $license_no            = $request->get( 'license_no' );
        if ( empty( $license_no ) ) {
            Session::flash( 'error', 'Invalid License No [MNOR-006]' );
        ...
        if ( $request->get( 'app_id' ) ) {
            ...
            $appData     = new MNOLicenseRenew();
            $processData = new ProcessList();
        }


        /** Fetch MNO License Issue existing data and prepare them for renew application */
        ...
        if ( $processData->status_id == 2 ) {
            $appInfo = [
                ...
                'process_type_name' => 'MNO License Renew',
                'remarks'           => '',
            ];
        ...
        if ( $processData->status_id == - 1 ) {
            ...
            Session::flash( 'error', 'Failed due to Application Status Conflict. Please try again later! [MNOR-007]' );
        }

        return redirect( '/mno-license-renew/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['appInfo'] = ProcessList::leftJoin( 'mno_license_renew as apps', 'apps.id', '=', 'process_list.ref_id' )
        ...
        $data['appShareholderInfo'] = self::Join( 'shareholders', 'shareholders.app_id', '=', 'mno_license_renew.id' )
            ->where( [
                'mno_license_renew.id'        => $decodedAppId,
        ...
        $public_html       = (string) view( 'REUSELicenseIssue::MNO.Renew.view', $data );
...
//                'apps.per_office_address as op_office_address',
        ...
        if ( $data['appInfo']->issue_tracking_no ) {
            $public_html = (string) view( 'REUSELicenseIssue::MNO.Renew.form-edit', $data );
        } else {
            $public_html = (string) view( 'REUSELicenseIssue::MNO.Renew.form-edit-v2', $data );
        }
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        $issue_company_id      = MNOLicenseIssue::where('license_no', $request->license_no)->value('company_id');
        $companyId             = CommonFunction::getUserCompanyWithZero();
        $data['companyInfo']   = CompanyInfo::where( 'is_approved', 1 )->where( 'id', $companyId )->first();
        ...
        $data['appInfo'] = ProcessList::leftJoin( 'mno_license_issue as apps', 'apps.id', '=', 'process_list.ref_id' )
            ->leftJoin( 'mno_license_master as ms', function ( $join ) use ( $process_type_id ) {
                $join->on( 'ms.issue_tracking_no', '=', 'apps.tracking_no' );
            } )
            ->leftJoin( 'process_status as ps', function ( $join ) use ( $process_type_id ) {
                $join->on( 'ps.id', '=', 'process_list.status_id' );
                $join->on( 'ps.process_type_id', '=', DB::raw( 58) );
            } )
            ->leftJoin( 'sp_payment as sfp', function ( $join ) use ( $process_type_id ) {
                $join->on( 'sfp.app_id', '=', 'process_list.ref_id' );
                $join->on( 'sfp.process_type_id', '=', DB::raw( 58 ) );       // comes for renew, so have to have payment for issue -> 1
            } )
            ->where( 'ms.license_no', $request->license_no )
            ->where( 'ms.status', 1 )
            ->where( 'process_list.process_type_id', 58 )                       // approved status can be renew
            ->first( [
                'process_list.id as process_list_id',
                'process_list.desk_id',
                'process_list.process_type_id',
...
//             return response()->json( [ 'responseCode' => 1, 'html' => $public_html ] );
        ...
        $shareholders_data = Shareholder::where( [
            'app_id'          => $data['appInfo']['id'],
            'process_type_id' => $this->mno_issue_process_type_id
        ] )->get( [
            'shareholders.id as shareholders_id',
        ...
        $contact_data = ContactPerson::where([
            ...
            'process_type_id' => $this->mno_issue_process_type_id // for mno license issue
        ])->get();
...
//        dd($data['contact_person']);
        ...
        $public_html = (string) view( 'REUSELicenseIssue::MNO.Renew.search', $data );

        return response()->json( [ 'responseCode' => 1, 'html' => $public_html ] );
    }

    private function fetchAndPrprIssueData( $license_no, $appData, $request ) {
        $existedData = MNOLicenseMaster::leftJoin( 'mno_license_issue as apps', 'apps.tracking_no', '=', 'mno_license_master.issue_tracking_no' )
            ->where( [ 'mno_license_master.license_no' => $license_no ] )->first();
        ...
        if ( $request->hasFile( 'declaration_q3_images' ) ) {
            ...
            $path      = 'uploads/mno-license-renew/' . $yearMonth;
            if ( ! file_exists( $path ) ) {
        ...
        $appData->declaration_q4           = $request->get( 'declaration_q4' );
        $appData->q4_license_number        = $request->get( 'q4_license_number' );
        $appData->q4_case_no               = $request->get( 'q4_case_no' );
        $appData->q4_amount                = $request->get( 'q4_amount' );
        $appData->q4_bank_draft_no         = $request->get( 'q4_bank_draft_no' );
        $appData->q4_given_comission       = $request->get( 'q4_given_comission' );
