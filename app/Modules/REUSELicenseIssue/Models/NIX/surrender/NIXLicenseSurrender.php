...
namespace App\Modules\REUSELicenseIssue\Models\NIX\surrender;
...
class NIXLicenseSurrender extends Model implements FormInterface {
    protected $table = 'nix_license_surrender';
    ...
    private $master_table = 'nix_license_master';
    ...
    public function createForm($currentInstance): string
    {
        ...
        return strval( view( "REUSELicenseIssue::NIX.Surrender.master", $data ) );
    }
    ...
    public function storeForm($request, $currentInstance): RedirectResponse
    {
        ...
        if ( $request->get( 'app_id' ) ) {
            $appData     = NIXLicenseSurrender::find( Encryption::decodeId($request->get('app_id')));
            $processData = ProcessList::where( [
                'process_type_id' => $this->process_type_id,
                'ref_id'          => $appData->id
            ] )->first();
        } else {
            $appData     = new NIXLicenseSurrender();
        ...
        $appData->surrender_date      = ! empty( $request->get( 'surrender_date' ) ) ? date( 'Y-m-d', strtotime( $request->get( 'surrender_date' ) ) ) : null;
        $appData->reason_of_surrender = $request->get( 'reason_of_surrender' );
        $appData->issue_tracking_no   = ! empty( $request->get( 'issue_tracking_no' ) ) ? Encryption::decodeId( $request->get( 'issue_tracking_no' ) ) : null;
        $appData->renew_tracking_no   = ! empty( $request->get( 'renew_tracking_no' ) ) ? Encryption::decodeId( $request->get( 'renew_tracking_no' ) ) : null;
...
//            }
        ...
        if ( $request->get( 'actionBtn' ) == 'submit' && $processData->status_id == 1 ) {
            if ( empty( $processData->tracking_no ) ) {
                $trackingPrefix = 'NIXS-' . date( 'Ymd' ) . '-';
                commonFunction::generateTrackingNumber( $trackingPrefix, $this->process_type_id, $processData->id, $appData->id, 'nix_license_renew' );
            }
        }
    ...
    public function viewForm($processTypeId, $applicationId): JsonResponse
    {
        ...
        $data['appInfo']         = ProcessList::leftJoin( 'nix_license_surrender as apps', 'apps.id', '=', 'process_list.ref_id' )
...
//                    }
        ...
        $public_html = (string) view( 'REUSELicenseIssue::NIX.Surrender.view', $data );
    ...
    public function editForm($processTypeId, $applicationId): JsonResponse
    {
        ...
        $public_html = (string) view( 'REUSELicenseIssue::NIX.Surrender.edit', $data );
    ...
    public function fetchData($request, $currentInstance): JsonResponse{
        ...
        $public_html = (string) view( 'REUSELicenseIssue::NIX.Surrender.search', $data );
