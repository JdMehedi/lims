...
class IGWLicenseIssue extends Model implements FormInterface {

    protected $table = 'igw_license_issue';
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval( view( 'REUSELicenseIssue::IGW.Issue.master', $data ));
    ...
    public function storeForm($request, $currentInstance): RedirectResponse
    {
        ...
        if ($request->get('app_id')) {
            $appData     = IGWLicenseIssue::find(Encryption::decodeId($app_id));
            $processData = ProcessList::where([
                'process_type_id' => $currentInstance->process_type_id,
                'ref_id' => $appData->id
            ])->first();
        } else {
            $appData     = new IGWLicenseIssue();
        ...
        if ((isset($request->payment_type) || $processData->status_id != $this->re_submit_status_id) && !empty($appData->igw_license_type)) {
            $unfixed_amount_array = $this->unfixedAmountsForGovtServiceFee($appData->igw_license_type, 1);
            $contact_info         = [
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('IGW', $this->process_type_id, $processData->id, $this->table, 'ISS', $appData->id);
        }
...
//
        ...
        if ($processData->status_id == $this->re_submit_status_id) {
            ...
            $loginControllerInstance->SendSmsService('APP_RESUBMIT', ['{$serviceName}' => 'IGW License Issue', '{$trackingNumber}' => $processData->tracking_no], $userMobile);
        ...
        return redirect('client/igw-license-issue/list/' . Encryption::encodeId($this->process_type_id));
    }
...
//        }
        ...
        $public_html = (string) view( "REUSELicenseIssue::IGW.Issue.masterView", $data );
    ...
    public function editForm( $decoded_process_type_id, $applicationId ): JsonResponse {
        ...
        $public_html = (string) view( 'REUSELicenseIssue::IGW.Issue.masterEdit', $data );
    ...
    private function storeLicenseData( $LicenseIssueObj, $request ) {
        ...
        if ( $request->hasFile( 'declaration_q3_images' ) ) {
            ...
            $path      = 'uploads/igw-license-issue/' . $yearMonth;
            if ( ! file_exists( $path ) ) {
