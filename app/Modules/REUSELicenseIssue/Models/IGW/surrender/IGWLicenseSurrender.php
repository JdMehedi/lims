...
namespace App\Modules\REUSELicenseIssue\Models\IGW\surrender;
...
class IGWLicenseSurrender extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'igw_license_cancellation';
    ...
    private $formPath = 'IGW.Surrender';
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval(view("REUSELicenseIssue::IGW.Surrender.form", $data));
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        if ( empty( $license_no ) ) {
            Session::flash( 'error', 'Invalid License No [IGWR-006]' );
        ...
        $appData = $this->storeIGWSurrenderData( $appData, $request );
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('IGW', $this->process_type_id, $processData->id, $this->table, 'SUR', $appData->id);
        }
        ...
        if ( $processData->status_id == 2 ) {
            $appInfo = [
                ...
                'process_type_name' => 'IGW License Surrender',
                'remarks'           => '',
            ];
        ...
        return redirect( '/igw-license-surrender/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        // for sub-view
        $shareholders_data = IGWLicenseSurrender::Join( 'shareholders', 'shareholders.app_id', '=', 'igw_license_cancellation.id' )
            ->where( [
                'igw_license_cancellation.id'     => $decodedAppId,
    ...
    public function editForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['districts'] = [ '' => 'Select' ] + Area::where( 'area_type', 2 )->orderBy( 'area_nm', 'ASC' )->pluck( 'area_nm', 'area_id' )->toArray();
        $data['appInfo']   = ProcessList::leftJoin( 'igw_license_cancellation as apps', 'apps.id', '=', 'process_list.ref_id' )
            ->leftJoin( 'process_status as ps', function ( $join ) use ( $process_type_id ) {
                $join->on( 'ps.id', '=', 'process_list.status_id' );
                $join->on( 'ps.process_type_id', '=', DB::raw( $process_type_id ) );
            } )
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        if ( ! empty( $data['master_data']->renew_tracking_no ) ) {
            $data['appInfo'] = ProcessList::leftJoin( 'igw_license_renew as apps', 'apps.id', '=', 'process_list.ref_id' )
                ->leftJoin( "igw_license_master as ms", function ( $join ){
        ...
        $public_html = (string) view( 'REUSELicenseIssue::IGW.Surrender.search', $data );

        return response()->json( [ 'responseCode' => 1, 'html' => $public_html ] );
    }

    private function storeIGWSurrenderData( $issueObj, $request ) {
