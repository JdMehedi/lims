...
namespace App\Modules\REUSELicenseIssue\Models\NTTN\amendment;
...
class NTTNLicenseAmendment extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'nttn_license_amendment';
    ...
    public function createForm($currentInstance): string
    {
        ...
        return strval( view( "REUSELicenseIssue::NTTN.Amendment.master", $data ) );
    }
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        if ( $request->get( 'app_id' ) ) {
            $appData     = NTTNLicenseAmendment::find( Encryption::decodeId( $request->get( 'app_id' ) ) );
            $processData = ProcessList::where( [
                'process_type_id' => $this->process_type_id,
                'ref_id'          => $appData->id
            ] )->first();
        } else {
            $appData     = new NTTNLicenseAmendment();
        ...
        if ( $request->get( 'actionBtn' ) == 'submit' && $processData->status_id == 1 ) {
            ...
            //Send SMS
            $loginControllerInstance->SendSmsService('APP_SUBMIT', ['{$serviceName}' => 'NTTN License Amendment', '{$trackingNumber}' => $trackingNumber], $userMobile);
            ...
            $appInfo = [
                ...
                'process_type_name' => 'NTTN License Amendment',
                'remarks'           => ''
            ];
        ...
        if ( $processData->status_id == 2 ) {
            ...
            //Send SMS
            $loginControllerInstance->SendSmsService('APP_RESUBMIT', ['{$serviceName}' => 'NTTN License Amendment', '{$trackingNumber}' => $trackingNumber], $userMobile);
...
//            $receiverInfo = CommonFunction::getCompanyUsersEmailPhone();
        ...
        return redirect( 'client/nttn-license-amendment/list/' . Encryption::encodeId( $currentInstance->process_type_id ) );
    }
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['appInfo'] = ProcessList::leftJoin( 'nttn_license_amendment as apps', 'apps.id', '=', 'process_list.ref_id' )
...
//        }
        ...
        $public_html = (string) view( 'REUSELicenseIssue::NTTN.Amendment.masterView', $data );
    ...
    public function editForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $public_html = (string) view( 'REUSELicenseIssue::NTTN.Amendment.masterEdit', $data );
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        if ($data['master_data']->renew_tracking_no) {
            $data['appInfo'] = ProcessList::leftJoin( 'nttn_license_renew as apps', 'apps.id', '=', 'process_list.ref_id' )
                ->leftJoin('nttn_license_master as ms',
...
//                    'apps.declaration_q3 as declaration_q3'
        ...
        $public_html = (string) view( 'REUSELicenseIssue::NTTN.Amendment.search', $data );
        return response()->json( [ 'responseCode' => 1, 'html' => $public_html ] );
    }
