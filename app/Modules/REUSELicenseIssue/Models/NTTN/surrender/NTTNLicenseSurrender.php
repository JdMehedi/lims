...
namespace App\Modules\REUSELicenseIssue\Models\NTTN\surrender;
...
class NTTNLicenseSurrender extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'nttn_license_surrender';
    protected $guarded = [ 'id' ];
    protected $process_type_id;
    protected $master_table = 'nttn_license_master';
    ...
    public function createForm($currentInstance): string
    {
        ...
        return strval( view( "REUSELicenseIssue::NTTN.Surrender.form", $data ) );
    }
    ...
    public function storeForm($request, $currentInstance): RedirectResponse
    {
        ...
        if ( empty( $license_no ) ) {
            Session::flash( 'error', "Invalid License No [NTTNS-006]" );

            return redirect()->back()->withInput();
        }

        if ( $request->get( 'app_id' ) ) {
            $appData     = NTTNLicenseSurrender::find( Encryption::decodeId($request->get('app_id')) );
            $processData = ProcessList::where( [
                'process_type_id' => $currentInstance->process_type_id,
                'ref_id'          => $appData->id
            ] )->first();
        } else {
            $appData     = new NTTNLicenseSurrender();
            $processData = new ProcessList();
        }


        $appData = $this->storeNTTNSurrenderData( $appData, $request );
        ...
        if ( $request->get( 'actionBtn' ) == 'submit' && $processData->status_id == 1 ) {
            ...
            if ( empty( $processData->tracking_no ) ) {
                $trackingPrefix = 'NTTN-S-' . date( "Ymd" ) . '-';
            ...
            //Send SMS
            $loginControllerInstance->SendSmsService('APP_SUBMIT', ['{$serviceName}' => 'NTTN License Surrender', '{$trackingNumber}' => $trackingNumber], $userMobile);
...
//            CommonFunction::sendEmailForReSubmission( $processData );
            ...
            //Send SMS
            $loginControllerInstance->SendSmsService('APP_RESUBMIT', ['{$serviceName}' => 'NTTN License Surrender', '{$trackingNumber}' => $trackingNumber], $userMobile);
...
//            $receiverInfo = CommonFunction::getCompanyUsersEmailPhone();
        ...
        return redirect( 'client/nttn-license-cancellation/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
    ...
    public function viewForm($processTypeId, $applicationId): JsonResponse
    {
        ...
        $data['appInfo']         = ProcessList::leftJoin( 'nttn_license_surrender as apps', 'apps.id', '=', 'process_list.ref_id' )
...
//                    }
        ...
        $public_html = (string) view( 'REUSELicenseIssue::NTTN.Surrender.view', $data );
    ...
    public function editForm($processTypeId, $applicationId): JsonResponse
    {
        ...
        $public_html = (string) view( 'REUSELicenseIssue::NTTN.Surrender.edit', $data );
    ...
    public function fetchData($request, $currentInstance): JsonResponse{
        ...
        $data['master_data'] = NTTNLicenseMaster::where( 'license_no', $request->license_no )->first();
        ...
        if ( ! empty( $data['master_data']->renew_tracking_no ) ) {
            $data['appInfo'] = ProcessList::leftJoin( 'nttn_license_renew as apps', 'apps.id', '=', 'process_list.ref_id' )
                ...
                ->leftJoin( 'process_status as ps', function ( $join ) {
                    $join->on( 'ps.id', '=', 'process_list.status_id' );
                    $join->on( 'ps.process_type_id', '=', DB::raw( 51 ) );
                } )
                ->leftJoin( 'sp_payment as sfp', function ( $join ) {
                    $join->on( 'sfp.app_id', '=', 'process_list.ref_id' );
                    $join->on( 'sfp.process_type_id', '=', DB::raw( 52 ) );
                } )
                ->where( 'ms.license_no', $request->get('license_no') )
                ->where( 'ms.status', 1 )
                ->where( 'process_list.process_type_id', 51 )
                ->first( [
                    'process_list.id as process_list_id',
                    'process_list.desk_id',
                    'process_list.process_type_id',
...
//                    'apps.declaration_q3 as declaration_q3',
            ...
            $data['appInfo']    = ProcessList::leftJoin( 'nttn_license_issue as apps', 'apps.id', '=', 'process_list.ref_id' )
                ...
                ->leftJoin( 'process_status as ps', function ( $join )  {
                    $join->on( 'ps.id', '=', 'process_list.status_id' );
                    $join->on( 'ps.process_type_id', '=', DB::raw( 50 ) );
                } )
                ->leftJoin( 'sp_payment as sfp', function ( $join ) {
                    $join->on( 'sfp.app_id', '=', 'process_list.ref_id' );
                    $join->on( 'sfp.process_type_id', '=', DB::raw( 52 ) );
                ...
                ->where( 'ms.license_no', $request->get('license_no') )
                ->where( 'ms.status', 1 )
                ->where( 'process_list.process_type_id', 50 )
                ->first( [
                    'process_list.id as process_list_id',
                    'process_list.desk_id',
                    'process_list.process_type_id',
...
//        dd($data['appInfo']);
        ...
        $public_html = (string) view( 'REUSELicenseIssue::NTTN.Surrender.search', $data );

        return response()->json( [ 'responseCode' => 1, 'html' => $public_html ] );
    }

    private function storeNTTNSurrenderData( $issueObj, $request ) {
