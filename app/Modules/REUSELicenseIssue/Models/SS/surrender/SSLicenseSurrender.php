...
namespace App\Modules\REUSELicenseIssue\Models\SS\surrender;
...
class  SSLicenseSurrender extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'ss_license_surrender';
    ...
    private $formPath = 'SS.Surrender';



    public function createForm( $currentInstance ): string {
        $this->assignProcessTypeId($currentInstance->process_type_id);
        ...
        return strval(view("REUSELicenseIssue::SS.Surrender.form", $data));
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        $appData = $this->storeSSLSurrenderData( $appData, $request );
...
//        dd($request->get( 'actionBtn' ));
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('SS', $this->process_type_id, $processData->id, $this->table, 'SUR', $appData->id);
        }
...
//            return SonaliPaymentController::RedirectToPaymentPortal( Encryption::encodeId( $payment_id ) );
        ...
        if ( $processData->status_id == 2 ) {
            $appInfo = [
                ...
                'process_type_name' => 'SS License Surrender',
                'remarks'           => '',
            ];
        ...
        if ( $processData->status_id == - 1 ) {
            ...
            Session::flash( 'error', 'Failed due to Application Status Conflict. Please try again later! [SSR-007]' );
        }

        return redirect( '/ss-license-surrender/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        // for sub-view
        $shareholders_data = SSLicenseSurrender::Join( 'shareholders', 'shareholders.app_id', '=', 'ss_license_surrender.id' )
            ->where( [
                'ss_license_surrender.id'     => $decodedAppId,
    ...
    public function editForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['districts'] = [ '' => 'Select' ] + Area::where( 'area_type', 2 )->orderBy( 'area_nm', 'ASC' )->pluck( 'area_nm', 'area_id' )->toArray();
        $data['appInfo'] = ProcessList::leftJoin( 'ss_license_surrender as apps', 'apps.id', '=', 'process_list.ref_id' )
                            ->leftJoin( 'process_status as ps', function ( $join ) use ( $process_type_id ) {
                                $join->on( 'ps.id', '=', 'process_list.status_id' )
                            ...
                            ->leftJoin( 'sp_payment as sfp', function ( $join ) use ( $process_type_id ) {
                                $join->on( 'sfp.app_id', '=', 'process_list.ref_id' )
                                     ->where( 'sfp.process_type_id', $process_type_id);
                            } )
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        $data['encoded_process_type_id'] = Encryption::encodeId($this->process_type_id);
        ...
        $data['master_data'] = SSLicenseMaster::where( 'license_no', $request->license_no )->first();
        ...
        if ( !empty( $data['master_data']->renew_tracking_no ) ) {
            ...
            $data['appInfo'] = ProcessList::leftJoin( 'ss_license_issue as apps', 'apps.id', '=', 'process_list.ref_id' )
                ...
                ->leftJoin( 'process_status as ps', function ( $join )  {
                    $join->on( 'ps.id', '=', 'process_list.status_id' )
                         ->where( 'ps.process_type_id', 78);
                } )
                ->leftJoin( 'sp_payment as sfp', function ( $join ) {
                    $join->on( 'sfp.app_id', '=', 'process_list.ref_id' )
                         ->where( 'sfp.process_type_id', 78);
                } )
                ->where( 'ms.license_no', $request->get('license_no') )
                ->where( 'ms.status', 1 )
                ->where( 'process_list.process_type_id', 78)
                ->first( [
                    'process_list.id as process_list_id',
                    'process_list.desk_id',
                    'process_list.process_type_id',
        ...
        $data['encoded_app_id'] = Encryption::encodeId($data['appInfo']->id);
        ...
        $public_html = (string) view( 'REUSELicenseIssue::SS.Surrender.search', $data );

        return response()->json( [ 'responseCode' => 1, 'html' => $public_html ] );
    }

    private function storeSSLSurrenderData( $issueObj, $request ) {
        ...
        if (!empty($request->get( 'declaration_image_file'))) {
            $issueObj->declaration_q3_doc = $request->get('declaration_image_file');
        }
    ...
    private function assignProcessTypeId($process_type_id) {
