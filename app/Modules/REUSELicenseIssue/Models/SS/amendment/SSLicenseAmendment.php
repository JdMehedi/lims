...
namespace App\Modules\REUSELicenseIssue\Models\SS\amendment;
...
class SSLicenseAmendment extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'ss_license_amendment';
    ...
    public function createForm( $currentInstance ): string {
       ...
       return strval(view("REUSELicenseIssue::SS.Amendment.master", $data));
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        if ( $request->get( 'app_id' ) ) {
            ...
            $appData     = new SSLicenseAmendment();
        ...
        if ( $appData->id ) {
            ...
            if ( $request->get( 'actionBtn' ) == 'draft' ) {
                ...
                if ( $processData->status_id == 5 ) { // For shortfall
                    ...
                    $processData->desk_id   = $request->get('license_no');
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('SS', $this->process_type_id, $processData->id, $this->table, 'AMM', $appData->id);
        }
        //=================================================payment code==========================
        ...
        if ( $processData->status_id == 2 ) {
            $appInfo = [
                ...
                'process_type_name' => 'SS License Amendment',
                'remarks'           => '',
            ];
        ...
        if ( $processData->status_id == - 1 ) {
            Session::flash( 'success', 'Successfully updated the SS Amendment Application!' );
        } elseif ( $processData->status_id == 1 ) {
            Session::flash( 'success', 'Successfully SS Amendment Application Submitted !' );
        } elseif ( $processData->status_id == 2 ) {
            Session::flash( 'success', 'Successfully SS Amendment Application Re-Submitted !' );
        } else {
            Session::flash( 'error', 'Failed due to Application Status Conflict. Please try again later! [ISPR-007]' );
        }

        return redirect( '/ss-license-amendment/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['appInfo'] = ProcessList::leftJoin( 'ss_license_amendment as apps', 'apps.id', '=', 'process_list.ref_id' )
        ...
        $public_html = (string) view( 'REUSELicenseIssue::SS.Amendment.masterView', $data );
    ...
    public function editForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['appInfo']   = ProcessList::leftJoin( 'ss_license_amendment as apps', 'apps.id', '=', 'process_list.ref_id' )
            ...
            ->first( [
                ...
                'ps.color',
                'apps.*',
                'apps.dd_file_1 as declaration_q1',
                'apps.dd_file_2 as declaration_q2',
                'apps.dd_file_3 as declaration_q3',
        ...
        /** Fetch data from ss_license_contact_person */
        $data['contact_person'] = ContactPerson::where( [
            'app_id'          => $applicationId,
            'process_type_id' => $process_type_id
        ] )->get();
        ...
        $public_html = (string) view( 'REUSELicenseIssue::SS.Amendment.masterEdit', $data );
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        $data['master_data'] = SSLicenseMaster::where('license_no', $request->license_no)->orderby('id', 'desc')->first();
        if (!empty($data['master_data']->renew_tracking_no)) {
            $data['appInfo'] = ProcessList::leftJoin( 'ss_license_renew as apps', 'apps.id', '=', 'process_list.ref_id' )
                ->leftJoin('ss_license_master as ms',
                ...
                ->leftJoin('process_status as ps',
                    function ( $join ) use ( $process_type_id) {
                        $join->on( 'ps.id', '=', 'process_list.status_id' );
                        $join->on( 'ps.process_type_id', '=', DB::raw( 79));
                    })
                ->leftJoin('sp_payment as sfp',
                    function ( $join ) use ( $process_type_id ) {
                        $join->on( 'sfp.app_id', '=', 'process_list.ref_id');
                        $join->on( 'sfp.process_type_id', '=', DB::raw(79));
                    })
            ...
            $data['appInfo'] = ProcessList::leftJoin( 'ss_license_issue as apps', 'apps.id', '=', 'process_list.ref_id' )
                ...
                ->leftJoin('sp_payment as sfp',
                    function ( $join ) use ( $process_type_id ) {
                        $join->on( 'sfp.app_id', '=', 'process_list.ref_id');
                        $join->on( 'sfp.process_type_id', '=', DB::raw(78));
                    })
                ->where( 'ms.license_no', $request->license_no )
                ->where( 'ms.status', 1 )
        ...
        $public_html = (string) view( 'REUSELicenseIssue::SS.Amendment.search', $data );
    ...
    private function storeLicenseData( $LicenseAmendmentObj, $request ) {
        ...
        if ( $request->hasFile( 'list_of_clients' ) ) {
            ...
            $path      = 'uploads/ss-license-ammendment/' . $yearMonth;
            if ( ! file_exists( $path ) ) {
