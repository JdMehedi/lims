...
namespace App\Modules\REUSELicenseIssue\Models\ITC\amendment;
...
class ITCLicenseAmendment extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'itc_license_amendment';
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval(view("REUSELicenseIssue::ITC.Amendment.master", $data));
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        if ( $request->get( 'app_id' ) ) {
            ...
            $appData     = new ITCLicenseAmendment();
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('ITC', $this->process_type_id, $processData->id, $this->table, 'AMM', $appData->id);
        }
        //=================================================payment code==========================
        ...
        if ( $processData->status_id == 2 ) {
            $appInfo = [
                ...
                'process_type_name' => 'ITC License Amendment',
                'remarks'           => '',
            ];
        ...
        if ( $processData->status_id == - 1 ) {
            Session::flash( 'success', 'Successfully updated the ITC Amendment Application!' );
        } elseif ( $processData->status_id == 1 ) {
            Session::flash( 'success', 'Successfully ITC Amendment Application Submitted !' );
        } elseif ( $processData->status_id == 2 ) {
            Session::flash( 'success', 'Successfully ITC Amendment Application Re-Submitted !' );
        } else {
            Session::flash( 'error', 'Failed due to Application Status Conflict. Please try again later! [ISPR-007]' );
        }

        return redirect( '/itc-license-amendment/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['appInfo'] = ProcessList::leftJoin( 'itc_license_amendment as apps', 'apps.id', '=', 'process_list.ref_id' )
        ...
        $public_html = (string) view( 'REUSELicenseIssue::ITC.Amendment.masterView', $data );
    ...
    public function editForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $public_html = (string) view( 'REUSELicenseIssue::ITC.Amendment.masterEdit', $data );
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        if (!empty($data['master_data']->renew_tracking_no)) {
            $data['appInfo'] = ProcessList::leftJoin( 'itc_license_renew as apps', 'apps.id', '=', 'process_list.ref_id' )
                ->leftJoin('itc_license_master as ms',
        ...
        $public_html = (string) view( 'REUSELicenseIssue::ITC.Amendment.search', $data );
    ...
    private function storeLicenseData( $LicenseAmendmentObj, $request ) {
        ...
        if ( $request->hasFile( 'list_of_clients' ) ) {
            ...
            $path      = 'uploads/itc-license-ammendment/' . $yearMonth;
            if ( ! file_exists( $path ) ) {
