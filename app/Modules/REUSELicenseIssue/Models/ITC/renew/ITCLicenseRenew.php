...
namespace App\Modules\REUSELicenseIssue\Models\ITC\renew;
...
use App\Modules\ITCLicenseRenew\Models\ITCLicenseRenewEquipmentList;
...
class ITCLicenseRenew extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'itc_license_renew';
    protected $guarded = [ 'id' ];
    protected $process_type_id;
    private $itc_issue_process_type_id = 54;
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval( view( 'REUSELicenseIssue::ITC.Renew.form', $data ) );
    }
    ...
    private function storeLicenseData($LicenseIssueObj, $request)
    {
        ...
        if ($request->hasFile('list_of_clients')) {
            ...
            $path      = 'uploads/itc-license-issue/' . $yearMonth;
            if (!file_exists($path)) {
        ...
        if ($request->hasFile('declaration_q3_images')) {
            $yearMonth = date('Y') . '/' . date('m') . '/';
            $path      = 'uploads/itc-license-renew/' . $yearMonth;
            if (!file_exists($path)) {
                mkdir($path, 0777, true);
            }
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['appShareholderInfo'] = self::Join( 'shareholders', 'shareholders.app_id', '=', 'itc_license_renew.id' )
            ->where( [
                'itc_license_renew.id'        => $decodedAppId,
        ...
        $public_html       = (string) view( 'REUSELicenseIssue::ITC.Renew.view', $data );
    ...
    public function editForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        if ( $data['appInfo']->issue_tracking_no ) {
            $public_html = (string) view( 'REUSELicenseIssue::ITC.Renew.form-edit', $data );
        } else {
            $public_html = (string) view( 'REUSELicenseIssue::ITC.Renew.form-edit-v2', $data );
        }
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        $data['appInfo'] = ProcessList::leftJoin( 'itc_license_issue as apps', 'apps.id', '=', 'process_list.ref_id' )
                                      ->leftJoin( 'itc_license_master as ms', function ( $join ) use ( $process_type_id ) {
        ...
        $shareholders_data = Shareholder::where([
            'app_id' => $data['appInfo']['id'],
            'process_type_id' => $this->itc_issue_process_type_id
        ])->get([
            'shareholders.id as shareholders_id',
        ...
        $data['contact_person']        = ContactPerson::where( [ 'app_id' => $data['appInfo']['id'], 'process_type_id' => $this->itc_issue_process_type_id ] )->get();
        ...
        $public_html = (string) view( 'REUSELicenseIssue::ITC.Renew.search', $data );
    ...
    private function fetchAndPrprIssueData( $license_no, $appData, $request ) {
        ...
        if ( $request->get( 'form_version' ) == 'v1' ) {
            $existedData = ITCLicenseMaster::leftJoin( 'itc_license_issue as apps', 'apps.tracking_no', '=', 'itc_license_master.issue_tracking_no' )
                                           ->where( [ 'itc_license_master.license_no' => $license_no ] )->first();
