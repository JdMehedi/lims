...
use App\Modules\REUSELicenseIssue\Models\ITC\issue\ISPLicenseIssue;
...
class ITCLicenseIssue extends Model implements FormInterface {

    protected $table = 'itc_license_issue';
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval( view( 'REUSELicenseIssue::ITC.Issue.master', $data ) );
    ...
    public function storeForm($request, $currentInstance): RedirectResponse
    {
        ...
        if ($request->get('app_id')) {
            $appData     = ITCLicenseIssue::find(Encryption::decodeId($app_id));
            $processData = ProcessList::where([
                'process_type_id' => $currentInstance->process_type_id,
                'ref_id' => $appData->id
            ])->first();
        } else {
            $appData     = new ITCLicenseIssue();
        ...
        if ((isset($request->payment_type) || $processData->status_id != $this->re_submit_status_id) && !empty($appData->itc_license_type)) {
            $unfixed_amount_array = $this->unfixedAmountsForGovtServiceFee($appData->itc_license_type, 1);
            $contact_info         = [
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('ITC', $this->process_type_id, $processData->id, $this->table, 'ISS', $appData->id);
        }
...
//            if ($request->get('payment_type') !== 'pay_order') {
        ...
        if ($processData->status_id == $this->re_submit_status_id) {
            ...
            $loginControllerInstance->SendSmsService('APP_RESUBMIT', ['{$serviceName}' => 'ITC License Issue', '{$trackingNumber}' => $processData->tracking_no], $userMobile);
        ...
        return redirect('client/itc-license-issue/list/' . Encryption::encodeId($this->process_type_id));
    }
...
//                'apps.noc_address2 as op_office_address2',
        ...
        $contact_person_data = ContactPerson::where( [ 'app_id' => $data['appInfo']['id'] ,'process_type_id' => 54  ] )->get(); // 5 = Call Center Issue Process type id
        ...
        $public_html = (string) view( "REUSELicenseIssue::ITC.Issue.masterView", $data );
    ...
    public function editForm( $decoded_process_type_id, $applicationId ): JsonResponse {
        ...
        /** Fetch data from itc_license_contact_person */
        ...
        $public_html = (string) view( 'REUSELicenseIssue::ITC.Issue.masterEdit', $data );
    ...
    private function unfixedAmountsForGovtServiceFee( $itc_license_type, $payment_step_id ) {
        ...
        $SpPaymentAmountConfData = SpPaymentAmountConf::where( [
            ...
            'license_type_id' => $itc_license_type,
            'status'          => 1,
        ] )->first();
