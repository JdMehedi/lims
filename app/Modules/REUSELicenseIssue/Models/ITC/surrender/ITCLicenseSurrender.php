...
namespace App\Modules\REUSELicenseIssue\Models\ITC\surrender;
...
use App\Modules\REUSELicenseIssue\Models\ITC\issue\ITCLicenseEquipmentList;
...
use App\Modules\REUSELicenseIssue\Models\ITC\issue\ITCLicenseTariffChart;
...
class ITCLicenseSurrender extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;


    protected $table = 'itc_license_cancellation';
    ...
    private $formPath = 'ITC.Surrender';
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval(view("REUSELicenseIssue::ITC.Surrender.form", $data));
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        if ( empty( $license_no ) ) {
            Session::flash( 'error', 'Invalid License No [ITCR-006]' );
        ...
        $appData = $this->storeITCSurrenderData( $appData, $request );
...
//        dd($request->get( 'actionBtn' ));
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('ITC', $this->process_type_id, $processData->id, $this->table, 'SUR', $appData->id);
        }
...
//            return SonaliPaymentController::RedirectToPaymentPortal( Encryption::encodeId( $payment_id ) );
        ...
        if ( $processData->status_id == 2 ) {
            $appInfo = [
                ...
                'process_type_name' => 'ITC License Surrender',
                'remarks'           => '',
            ];
        ...
        if ( $processData->status_id == - 1 ) {
            ...
            Session::flash( 'error', 'Failed due to Application Status Conflict. Please try again later! [ITCR-007]' );
        }

        return redirect( '/itc-license-renew/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['appDynamicDocInfo'] = ApplicationDocuments::where( 'process_type_id', $processTypeId )
            ...
            ->get();
        // for sub-view
        $shareholders_data = ITCLicenseSurrender::Join( 'shareholders', 'shareholders.app_id', '=', 'itc_license_cancellation.id' )
            ->where( [
                'itc_license_cancellation.id'     => $decodedAppId,
    ...
    public function editForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['districts'] = [ '' => 'Select' ] + Area::where( 'area_type', 2 )->orderBy( 'area_nm', 'ASC' )->pluck( 'area_nm', 'area_id' )->toArray();
        $data['appInfo']   = ProcessList::leftJoin( 'itc_license_cancellation as apps', 'apps.id', '=', 'process_list.ref_id' )
            ->leftJoin( 'process_status as ps', function ( $join ) use ( $process_type_id ) {
                $join->on( 'ps.id', '=', 'process_list.status_id' );
                $join->on( 'ps.process_type_id', '=', DB::raw( $process_type_id ) );
            } )
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        $issue_company_id      = ITCLicenseIssue::where('license_no', $request->license_no)->value('company_id');
        $companyId             = CommonFunction::getUserCompanyWithZero();
        $data['companyInfo']   = CompanyInfo::where( 'is_approved', 1 )->where( 'id', $companyId )->first();
        $data['companyUserType'] = CommonFunction::getCompanyUserType();
        ...
        $data['master_data'] = ITCLicenseMaster::where( 'license_no', $request->license_no )->first();


        if ( ! empty( $data['master_data']->renew_tracking_no ) ) {
            $data['appInfo'] = ProcessList::leftJoin( 'itc_license_renew as apps', 'apps.id', '=', 'process_list.ref_id' )
                ->leftJoin( "itc_license_master as ms", function ( $join ){
                    $join->on( 'ms.renew_tracking_no', '=', 'apps.tracking_no' );
                } )
                ->leftJoin( 'process_status as ps', function ( $join ) {
                    $join->on( 'ps.id', '=', 'process_list.status_id' );
                    $join->on( 'ps.process_type_id', '=', DB::raw( 55 ) );
                } )
                ->leftJoin( 'sp_payment as sfp', function ( $join ) {
                    $join->on( 'sfp.app_id', '=', 'process_list.ref_id' );
                    $join->on( 'sfp.process_type_id', '=', DB::raw( 55 ) );
                } )
                ->where( 'ms.license_no', $request->get('license_no') )
                ->where( 'ms.status', 1 )
                ->where( 'process_list.process_type_id', 55 )
                ->first( [
                    'process_list.id as process_list_id',
                    'process_list.desk_id',
                    'process_list.process_type_id',
                    ...
                    'apps.company_type as company_type',
            ...
            $data['appInfo']    = ProcessList::leftJoin( 'itc_license_issue as apps', 'apps.id', '=', 'process_list.ref_id' )
                ->leftJoin( "itc_license_master as ms", function ( $join ) {
                    $join->on( 'ms.issue_tracking_no', '=', 'apps.tracking_no' );
                } )
                ->leftJoin( 'process_status as ps', function ( $join )  {
                    $join->on( 'ps.id', '=', 'process_list.status_id' );
                    $join->on( 'ps.process_type_id', '=', DB::raw( 54 ) );
                } )
                ->leftJoin( 'sp_payment as sfp', function ( $join ) {
                    $join->on( 'sfp.app_id', '=', 'process_list.ref_id' );
                    $join->on( 'sfp.process_type_id', '=', DB::raw( 54 ) );
                } )
                ->where( 'ms.license_no', $request->get('license_no') )
                ->where( 'ms.status', 1 )
                ->where( 'process_list.process_type_id', 54 )
                ->first( [
                    'process_list.id as process_list_id',
                    'process_list.desk_id',
                    'process_list.process_type_id',
        ...
        $public_html = (string) view( 'REUSELicenseIssue::ITC.Surrender.search', $data );

        return response()->json( [ 'responseCode' => 1, 'html' => $public_html ] );
    }

    private function storeITCSurrenderData( $issueObj, $request ) {
        ...
        $issueObj->license_issue_date            = date( 'Y-m-d', strtotime( $request->issue_date ) );
        $issueObj->expiry_date           = date( 'Y-m-d', strtotime( $request->expiry_date ) );
