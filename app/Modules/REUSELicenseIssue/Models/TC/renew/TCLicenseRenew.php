...
namespace App\Modules\REUSELicenseIssue\Models\TC\renew;
...
class TCLicenseRenew extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'tc_license_renew';
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval( view( 'REUSELicenseIssue::TC.Renew.form', $data ) );
    }
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        if ( empty( $license_no ) ) {
            Session::flash( 'error', 'Invalid License No [TCR-006]' );
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('TC', $this->process_type_id, $processData->id, $this->table, 'REN', $appData->id);
        }
        ...
        if ( $processData->status_id == 2 ) {
            ...
            //Send SMS
//            $loginControllerInstance->SendSmsService('APP_RESUBMIT', ['{$serviceName}' => 'TC License Renew', '{$trackingNumber}' => $trackingNumber], $userMobile);
            ...
            $appInfo = [
                ...
                'process_type_name' => 'TC License Renew',
                'remarks'           => '',
            ];
        ...
        if ( $processData->status_id == - 1 ) {
            ...
            Session::flash( 'error', 'Failed due to Application Status Conflict. Please try again later! [TCR-007]' );
        }

        return redirect( '/tc-license-renew/list/' . Encryption::encodeId( $this->process_type_id ) );
    }


    private function fetchAndPrprIssueData( $license_no, $appData, $request ) {
        $existedData = TCLicenseMaster::leftJoin( 'tc_license_issue as apps', 'apps.tracking_no', '=', 'tc_license_master.issue_tracking_no' )
            ->where( [ 'tc_license_master.license_no' => $license_no ] )->first();
            ...
            if ( $request->hasFile( 'declaration_q3_images' ) ) {
                ...
                $path      = 'uploads/tc-license-renew/' . $yearMonth;
                if ( ! file_exists( $path ) ) {
            ...
            $appData->license_issue_date       = ! empty( $existedData['license_issue_date'] ) ? $existedData['license_issue_date'] : null;
...
//        }
        ...
        $public_html = (string) view( 'REUSELicenseIssue::TC.Renew.view', $data );
    ...
    public function editForm($processTypeId, $applicationId): JsonResponse
    {
            ...
            if ( empty( $NixRenewData->issue_tracking_no ) ) {
                $public_html = (string) view( 'REUSELicenseIssue::TC.Renew.masterEditV2', $data );
            } else {
                $public_html = (string) view( 'REUSELicenseIssue::TC.Renew.masterEdit', $data );
            }
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        $public_html = (string) view( 'REUSELicenseIssue::TC.Renew.search', $data );
