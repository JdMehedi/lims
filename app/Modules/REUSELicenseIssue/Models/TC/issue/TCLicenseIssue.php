...
namespace App\Modules\REUSELicenseIssue\Models\TC\issue;
...
class TCLicenseIssue extends Model implements FormInterface {

    protected $table = 'tc_license_issue';
    ...
    public function createForm( $currentInstance ): string {
        ...
        if( $data['companyInfo']->office_district){
            $data['thana']         = [ '' => 'Select' ] + Area::where( 'area_type', 3 )->where( 'pare_id', $data['companyInfo']->office_district )->orderBy( 'area_nm', 'ASC' )->pluck( 'area_nm', 'area_id' )->toArray();
        }
        ...
        return strval( view( 'REUSELicenseIssue::TC.Issue.master', $data ) );
    ...
    public function storeForm($request, $currentInstance): RedirectResponse
    {
        ...
        if ($request->get('app_id')) {
            $appData     = TCLicenseIssue::find(Encryption::decodeId($app_id));
            $processData = ProcessList::where([
                'process_type_id' => $currentInstance->process_type_id,
                'ref_id' => $appData->id
            ])->first();
        } else {
            $appData     = new TCLicenseIssue();
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('TC', $this->process_type_id, $processData->id, $this->table, 'ISS', $appData->id);
        }
...
//
        ...
        return redirect('client/tc-license-issue/list/' . Encryption::encodeId($this->process_type_id));
    }
...
//        }
        ...
        $contact_person_data = ContactPerson::where( [ 'app_id' => $data['appInfo']['id'] ,'process_type_id' => 66  ] )->get(); // 5 = Call Center Issue Process type id
...
//        $data['proposal_area'] = $proposal_area_data;
        ...
        $public_html = (string) view( "REUSELicenseIssue::TC.Issue.masterView", $data );
    ...
    public function editForm( $decoded_process_type_id, $applicationId ): JsonResponse {
        ...
        $public_html = (string) view( 'REUSELicenseIssue::TC.Issue.masterEdit', $data );
