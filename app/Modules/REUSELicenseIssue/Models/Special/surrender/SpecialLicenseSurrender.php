...
namespace App\Modules\REUSELicenseIssue\Models\Special\surrender;
...
class SpecialLicenseSurrender extends Model implements FormInterface
{

    protected $table = 'special_license_surrender';
    ...
    public function viewForm($processTypeId, $appId): JsonResponse
    {
        ...
        $application_data = SpecialLicenseSurrender::find($decodedAppId);
        
        $dynamic_form = DynamicProcess::where('process_type_id',$process_type_id)->first();
        $data['process_type_id'] = $process_type_id;

        $data['appInfo'] = ProcessList::leftJoin('special_license_surrender as apps', 'apps.id', '=', 'process_list.ref_id')
