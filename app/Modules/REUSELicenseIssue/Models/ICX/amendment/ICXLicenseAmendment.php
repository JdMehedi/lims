...
namespace App\Modules\REUSELicenseIssue\Models\ICX\amendment;
...
class ICXLicenseAmendment extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'icx_license_amendment';
    ...
    public function createForm($currentInstance): string
    {
        ...
        return strval( view( "REUSELicenseIssue::ICX.Amendment.master", $data ) );
    }
    ...
    public function storeForm($request, $currentInstance): RedirectResponse
    {
        ...
        if ( $request->get( 'app_id' ) ) {
            $appData     = ICXLicenseAmendment::find( Encryption::decodeId($request->get('app_id')));
            $processData = ProcessList::where( [
                'process_type_id' => $this->process_type_id,
                'ref_id'          => $appData->id
            ] )->first();
        } else {
            $appData     = new ICXLicenseAmendment();
        ...
        if ($request->hasFile('declaration_q3_images')) {
            ...
            $path = 'uploads/icx-license-amendment/' . $yearMonth;
            if (!file_exists($path)) {
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('ICX', $this->process_type_id, $processData->id, $this->table, 'AMM', $appData->id);
        }
        ...
        if ( $processData->status_id == 2 ) {
            $appInfo = [
                ...
                'process_type_name' => 'ICX License Amendment',
                'remarks'           => '',
            ];
        ...
        return redirect( 'client/icx-license-amendment/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
    ...
    public function viewForm($processTypeId, $applicationId): JsonResponse
    {
        ...
        $data['appInfo'] = ProcessList::leftJoin( 'icx_license_amendment as apps', 'apps.id', '=', 'process_list.ref_id' )
        ...
        $public_html = (string) view( 'REUSELicenseIssue::ICX.Amendment.masterView', $data );
    ...
    public function editForm($processTypeId, $applicationId): JsonResponse
    {
            ...
            $public_html = (string) view( 'REUSELicenseIssue::ICX.Amendment.masterEdit', $data );
    ...
    public function fetchData($request, $currentInstance): JsonResponse{
        ...
        if ( empty( $data['license_no'] ) ) {
            $public_html = (string) view( 'REUSELicenseIssue::ICX.Amendment.search-blank', $data );
        ...
        if (($data['master_data']!=null) && $data['master_data']->renew_tracking_no) {
            ...
            ->leftJoin( 'sp_payment as sfp', function ( $join ) use ( $issue_process_type_id ) {
                $join->on( 'sfp.app_id', '=', 'process_list.ref_id' );
                $join->on( 'sfp.process_type_id', '=', DB::raw( 33 ) );       // comes for renew, so have to have payment for issue -> 1
            } )
            ->where( 'ns.license_no', $request->license_no )
            ...
            ->first( [
                ...
                'ns.*',
                'apps.*',
                 'apps.company_org_name as company_name',
                'sfp.contact_name as sfp_contact_name',
                'sfp.contact_email as sfp_contact_email',
                'sfp.contact_no as sfp_contact_phone',
                'sfp.address as sfp_contact_address',
        ...
        $public_html = (string) view( 'REUSELicenseIssue::ICX.Amendment.search', $data );
