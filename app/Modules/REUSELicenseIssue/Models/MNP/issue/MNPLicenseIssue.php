...
namespace App\Modules\REUSELicenseIssue\Models\MNP\issue;
...
class MNPLicenseIssue extends Model implements FormInterface {

    protected $table = 'mnp_license_issue';
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval( view( 'REUSELicenseIssue::MNP.Issue.master', $data ));
    ...
    public function storeForm($request, $currentInstance): RedirectResponse
    {
        ...
        if ($request->get('app_id')) {
            $appData     = MNPLicenseIssue::find(Encryption::decodeId($app_id));
            $processData = ProcessList::where([
                'process_type_id' => $currentInstance->process_type_id,
                'ref_id' => $appData->id
            ])->first();
        } else {
            $appData     = new MNPLicenseIssue();
        ...
        if ((isset($request->payment_type) || $processData->status_id != $this->re_submit_status_id) && !empty($appData->mnp_license_type)) {
            $unfixed_amount_array = $this->unfixedAmountsForGovtServiceFee($appData->mnp_license_type, 1);
            $contact_info         = [
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('MNP', $this->process_type_id, $processData->id, $this->table, 'ISS', $appData->id);
        }
...
//
            ...
            // if ($request->get('payment_type') !== 'pay_order') {
                ...
                return SonaliPaymentController::RedirectToPaymentPortal(Encryption::encodeId(isset($payment_id)));
            }
        ...
        if ($processData->status_id == $this->re_submit_status_id) {
            ...
            $loginControllerInstance->SendSmsService('APP_RESUBMIT', ['{$serviceName}' => 'MNP License Issue', '{$trackingNumber}' => $processData->tracking_no], $userMobile);
        ...
        return redirect('client/mnp-license-issue/list/' . Encryption::encodeId($this->process_type_id));
    }
...
//        }
        ...
        $contact_person_data = ContactPerson::where( [ 'app_id' => $data['appInfo']['id'] ,'process_type_id' => 70  ] )->get(); // 5 = Call Center Issue Process type id
        ...
        $public_html = (string) view( "REUSELicenseIssue::MNP.Issue.masterView", $data );
    ...
    public function editForm( $decoded_process_type_id, $applicationId ): JsonResponse {
        ...
        $public_html = (string) view( 'REUSELicenseIssue::MNP.Issue.masterEdit', $data );
    ...
    private function storeLicenseData( $LicenseIssueObj, $request ) {
        ...
        if ( $request->hasFile( 'list_of_clients' ) ) {
            ...
            $path      = 'uploads/mnp-license-issue/' . $yearMonth;
            if ( ! file_exists( $path ) ) {
    ...
    private function storeProcessListData( $request, $processListObj, $appData ) {
        ...
        if ( $request->get( 'actionBtn' ) === 'draft' ) {
            ...
            // here would be draf_status_id while did payment
