...
namespace App\Modules\REUSELicenseIssue\Models\MNP\amendment;
...
class MNPLicenseAmendment extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'mnp_license_amendment';
    ...
    public function createForm( $currentInstance ): string {
       ...
       return strval(view("REUSELicenseIssue::MNP.Amendment.master", $data));
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        if ( $request->get( 'app_id' ) ) {
            ...
            $appData     = new MNPLicenseAmendment();
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('MNP', $this->process_type_id, $processData->id, $this->table, 'AMM', $appData->id);
        }
        //=================================================payment code==========================
        ...
        if ( $processData->status_id == 2 ) {
            $appInfo = [
                ...
                'process_type_name' => 'MNP License Amendment',
                'remarks'           => '',
            ];
        ...
        if ( $processData->status_id == - 1 ) {
            Session::flash( 'success', 'Successfully updated the MNP Amendment Application!' );
        } elseif ( $processData->status_id == 1 ) {
            Session::flash( 'success', 'Successfully MNP Amendment Application Submitted !' );
        } elseif ( $processData->status_id == 2 ) {
            Session::flash( 'success', 'Successfully MNP Amendment Application Re-Submitted !' );
        } else {
            Session::flash( 'error', 'Failed due to Application Status Conflict. Please try again later! [ISPR-007]' );
        }

        return redirect( '/mnp-license-amendment/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['appInfo'] = ProcessList::leftJoin( 'mnp_license_amendment as apps', 'apps.id', '=', 'process_list.ref_id' )
        ...
        $public_html = (string) view( 'REUSELicenseIssue::MNP.Amendment.masterView', $data );
...
//        ] )->get();
        ...
        $public_html = (string) view( 'REUSELicenseIssue::MNP.Amendment.masterEdit', $data );
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        if (!empty($data['master_data']->renew_tracking_no)) {
            $data['appInfo'] = ProcessList::leftJoin( 'mnp_license_renew as apps', 'apps.id', '=', 'process_list.ref_id' )
                ->leftJoin('mnp_license_master as ms',
        ...
        $public_html = (string) view( 'REUSELicenseIssue::MNP.Amendment.search', $data );
