...
namespace App\Modules\REUSELicenseIssue\Models\MNP\renew;
...
use App\Modules\MNPLicenseIssue\Models\MNPLicenseContactPerson;
...
class MNPLicenseRenew extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'mnp_license_renew';
    protected $guarded = [ 'id' ];
    protected $process_type_id;
    private $shortfall_status_id = 5;
    private $mnp_issue_process_type_id = 70;
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval( view( 'REUSELicenseIssue::MNP.Renew.form', $data ) );
    }
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['appShareholderInfo'] = self::Join( 'shareholders', 'shareholders.app_id', '=', 'mnp_license_renew.id' )
            ->where( [
                'mnp_license_renew.id'        => $decodedAppId,
        ...
        $public_html       = (string) view( 'REUSELicenseIssue::MNP.Renew.view', $data );
...
//                'apps.per_office_address as op_office_address',
        ...
        if ( $data['appInfo']->issue_tracking_no ) {
            $public_html = (string) view( 'REUSELicenseIssue::MNP.Renew.form-edit', $data );
        } else {
            $public_html = (string) view( 'REUSELicenseIssue::MNP.Renew.form-edit-v2', $data );
        }
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        $data['appInfo'] = ProcessList::leftJoin( 'mnp_license_issue as apps', 'apps.id', '=', 'process_list.ref_id' )
            ->leftJoin( 'mnp_license_master as ms', function ( $join ) use ( $process_type_id ) {
            ...
            ->leftJoin( 'sp_payment as sfp', function ( $join ) use ( $process_type_id ) {
                $join->on( 'sfp.app_id', '=', 'process_list.ref_id' );
                $join->on( 'sfp.process_type_id', '=', DB::raw( 70 ) );       // comes for renew, so have to have payment for issue -> 1
            } )
            ->where( 'ms.license_no', $request->license_no )
            ->where( 'ms.status', 1 )
            ->where( 'process_list.process_type_id', 70 )                       // approved status can be renew
            ->first( [
                'process_list.id as process_list_id',
                'process_list.desk_id',
                'process_list.process_type_id',
        ...
        $shareholders_data = Shareholder::where( [
            'app_id'          => $data['appInfo']['id'],
            'process_type_id' => $this->mnp_issue_process_type_id
        ] )->get( [
            'shareholders.id as shareholders_id',
        ...
        $contact_data = ContactPerson::where([
            ...
            'process_type_id' => $this->mnp_issue_process_type_id // for isp license issue
        ])->get();
        ...
        $public_html = (string) view( 'REUSELicenseIssue::MNP.Renew.search', $data );

        return response()->json( [ 'responseCode' => 1, 'html' => $public_html ] );
    }

    private function fetchAndPrprIssueData( $license_no, $appData, $request ) {
        $existedData = MNPLicenseMaster::leftJoin( 'mnp_license_issue as apps', 'apps.tracking_no', '=', 'mnp_license_master.issue_tracking_no' )
            ->where( [ 'mnp_license_master.license_no' => $license_no ] )->first();
        ...
        if ( $request->hasFile( 'declaration_q3_images' ) ) {
            ...
            $path      = 'uploads/mnp-license-renew/' . $yearMonth;
            if ( ! file_exists( $path ) ) {
