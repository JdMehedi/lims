...
namespace App\Modules\REUSELicenseIssue\Models\BWA\issue;
...
class BWALicenseIssue extends Model implements FormInterface {

    protected $table = 'bwa_license_issue';
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval( view( 'REUSELicenseIssue::BWA.Issue.master', $data ) );
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        if ( $request->get( 'app_id' ) ) {
            $appData     = BWALicenseIssue::find( Encryption::decodeId( $app_id ) );
            $processData = ProcessList::where( [
                'process_type_id' => $currentInstance->process_type_id,
                'ref_id'          => $appData->id
            ] )->first();
        } else {
            $appData     = new BWALicenseIssue();
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('PSTN', $this->process_type_id, $processData->id, $this->table, 'ISS', $appData->id);
        }
        ...
        return redirect( 'client/bwa-license-issue/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
...
//                'apps.noc_address2 as op_office_address2',
        ...
        $contact_person_data = ContactPerson::where( [ 'app_id' => $data['appInfo']['id'] ,'process_type_id' => $process_type_id  ] )->get(); // 5 = Call Center Issue Process type id
        ...
        $public_html = (string) view( "REUSELicenseIssue::BWA.Issue.masterView", $data );
    ...
    public function editForm( $decoded_process_type_id, $applicationId ): JsonResponse {
        ...
        /** Fetch data from bwa_license_contact_person */
        ...
        $public_html = (string) view( 'REUSELicenseIssue::BWA.Issue.masterEdit', $data );
    ...
    private function storeLicenseData( $LicenseIssueObj, $request ) {
        ...
        if ( $request->hasFile( 'list_of_clients' ) ) {
            ...
            $path      = 'uploads/bwa-license-issue/' . $yearMonth;
            if ( ! file_exists( $path ) ) {
