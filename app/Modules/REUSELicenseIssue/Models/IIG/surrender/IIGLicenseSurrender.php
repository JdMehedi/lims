...
namespace App\Modules\REUSELicenseIssue\Models\IIG\surrender;
...
class IIGLicenseSurrender extends Model implements FormInterface {

    protected $table = 'iig_license_cancellation';
    ...
    private $formPath = 'IIG.Surrender';
    private $master_table='iig_license_master';
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval(view("REUSELicenseIssue::IIG.Surrender.form", $data));
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        if ( empty( $license_no ) ) {
            Session::flash( 'error', 'Invalid License No [IIGR-006]' );
        ...
        $appData = $this->storeIIGSurrenderData( $appData, $request );
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('IIG', $this->process_type_id, $processData->id, $this->table, 'SUR', $appData->id);
        }
        ...
        if ( $processData->status_id == 2 ) {
            $appInfo = [
                ...
                'process_type_name' => 'IIG License Surrender',
                'remarks'           => '',
            ];
        ...
        if ( $processData->status_id == - 1 ) {
            ...
            Session::flash( 'error', 'Failed due to Application Status Conflict. Please try again later! [IIGR-007]' );
        }

        return redirect( '/iig-license-surrender/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        // for sub-view
        $shareholders_data = IIGLicenseSurrender::Join( 'shareholders', 'shareholders.app_id', '=', 'iig_license_cancellation.id' )
            ->where( [
                'iig_license_cancellation.id'     => $decodedAppId,
    ...
    public function editForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['appInfo']   = ProcessList::leftJoin( 'iig_license_cancellation as apps', 'apps.id', '=', 'process_list.ref_id' )
            ->leftJoin( 'process_status as ps', function ( $join ) use ( $process_type_id ) {
                $join->on( 'ps.id', '=', 'process_list.status_id' );
                $join->on( 'ps.process_type_id', '=', DB::raw( $process_type_id ) );
            } )
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        $public_html = (string) view( 'REUSELicenseIssue::IIG.Surrender.search', $data );

        return response()->json( [ 'responseCode' => 1, 'html' => $public_html ] );
    }

    private function storeIIGSurrenderData( $issueObj, $request ) {
