...
namespace App\Modules\REUSELicenseIssue\Models\IIG\amendment;
...
class IIGLicenseAmendment extends Model implements FormInterface {

    protected $table = 'iig_license_amendment';
    ...
    public function createForm( $currentInstance ): string {
        ...
        return strval( view( 'REUSELicenseIssue::IIG.Amendment.master', $data ) );
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        if ( $request->get( 'app_id' ) ) {
            ...
            $appData     = new IIGLicenseAmendment();
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('IIG', $this->process_type_id, $processData->id, $this->table, 'AMM', $appData->id);
        }
        //=================================================payment code==========================
        ...
        if ( $processData->status_id == 2 ) {
            $appInfo = [
                ...
                'process_type_name' => 'IIG License Amendment',
                'remarks'           => '',
            ];
        ...
        if ( $processData->status_id == - 1 ) {
            Session::flash( 'success', 'Successfully updated the IIG Amendment Application!' );
        } elseif ( $processData->status_id == 1 ) {
            Session::flash( 'success', 'Successfully IIG Amendment Application Submitted !' );
        } elseif ( $processData->status_id == 2 ) {
            Session::flash( 'success', 'Successfully IIG Amendment Application Re-Submitted !' );
        } else {
            Session::flash( 'error', 'Failed due to Application Status Conflict. Please try again later! [ISPR-007]' );
        }

        return redirect( '/iig-license-amendment/list/' . Encryption::encodeId( $this->process_type_id ) );
    }
    ...
    public function viewForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['appInfo'] = ProcessList::leftJoin( 'iig_license_amendment as apps', 'apps.id', '=', 'process_list.ref_id' )
        ...
        $public_html = (string) view( 'REUSELicenseIssue::IIG.Amendment.masterView', $data );
    ...
    public function editForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $public_html = (string) view( 'REUSELicenseIssue::IIG.Amendment.masterEdit', $data );
    ...
    private function storeLicenseData( $LicenseIssueObj, $request ) {
        ...
        if ( $request->hasFile( 'list_of_clients' ) ) {
            ...
            $path      = 'uploads/iig-license-ammendment/' . $yearMonth;
            if ( ! file_exists( $path ) ) {
    ...
    public function fetchData( $request, $currentInstance ): JsonResponse {
        ...
        $data['master_data'] = IIGLicenseMaster::where('license_no', $request->license_no)->first();
        if (!empty($data['master_data']->renew_tracking_no)) {
            $data['appInfo'] = ProcessList::leftJoin( 'iig_license_renew as apps', 'apps.id', '=', 'process_list.ref_id' )
                ->leftJoin('iig_license_master as ms',
                ...
                ->leftJoin('process_status as ps',
                    function ( $join ) use ( $process_type_id) {
                        $join->on( 'ps.id', '=', 'process_list.status_id' );
                        $join->on( 'ps.process_type_id', '=', DB::raw( 18));
                    })
                ->leftJoin('sp_payment as sfp',
                    function ( $join ) use ( $process_type_id ) {
                        $join->on( 'sfp.app_id', '=', 'process_list.ref_id');
                        $join->on( 'sfp.process_type_id', '=', DB::raw(18));
                    })
                ->where( 'ms.license_no', $request->license_no)
                ->where( 'ms.status', 1 )
                ->where('process_list.process_type_id', 18)
                ->first( [
                    'process_list.id as process_list_id',
                    'process_list.desk_id',
                    'process_list.process_type_id',
            ...
            $data['appInfo'] = ProcessList::leftJoin( 'iig_license_issue as apps', 'apps.id', '=', 'process_list.ref_id' )
                ...
                ->leftJoin('process_status as ps',
                    function ( $join ) use ( $process_type_id) {
                        $join->on( 'ps.id', '=', 'process_list.status_id' );
                        $join->on( 'ps.process_type_id', '=', DB::raw(17));
                    })
                ->leftJoin('sp_payment as sfp',
                    function ( $join ) use ( $process_type_id ) {
                        $join->on( 'sfp.app_id', '=', 'process_list.ref_id');
                        $join->on( 'sfp.process_type_id', '=', DB::raw(17));
                    })
                ->where( 'ms.license_no', $request->license_no )
                ->where( 'ms.status', 1 )
        ...
        $public_html = (string) view( 'REUSELicenseIssue::IIG.Amendment.search', $data );
