...
namespace App\Modules\REUSELicenseIssue\Models\IPTSP\amendment;
...
class IPTSPLicenseAmendment extends Model implements FormInterface {
    use SPPaymentManager;
    use SPAfterPaymentManager;

    protected $table = 'iptsp_license_amendment';
    ...
    public function createForm( $currentInstance ): string {
       ...
       // $data['existing_isp_license'] = ISPLicenseMaster::where('company_id',Auth::user()->working_company_id)->first();
        ...
        return strval(view("REUSELicenseIssue::IPTSP.Amendment.master", $data));
    ...
    public function storeForm( $request, $currentInstance ): RedirectResponse {
        ...
        if ($request->get('app_id')) {
            $appData     = IPTSPLicenseAmendment::find(Encryption::decodeId($app_id));
            $processData = ProcessList::where([
                'process_type_id' => $currentInstance->process_type_id,
                'ref_id' => $appData->id
            ])->first();
        } else {
            $appData     = new IPTSPLicenseAmendment();
        ...
        if (in_array($request->get('actionBtn'), ['draft', 'submit']) && empty($appData->tracking_no)) {
            CommonFunction::generateUniqueTrackingNumber('IPTSP', $this->process_type_id, $processData->id, $this->table, 'AMM', $appData->id);
        }


        /** if application submitted and status is equal to draft then generate tracking number and payment initiate  ***/
        if ( $request->get( 'actionBtn' ) == 'submit' && $this->status_id == 1 ) {
//            if ( empty( $processData->tracking_no ) ) {
//                $trackingPrefix = 'IPTSPA-' . date( 'Ymd' ) . '-';
...
//            }
            ...
            //Send SMS
            $loginControllerInstance->SendSmsService('APP_SUBMIT', ['{$serviceName}' => 'IPTSP License Amendment', '{$trackingNumber}' => $trackingNumber], $userMobile);
            ...
            $appInfo = [
                ...
                'process_type_name' => 'IPTSP License Amendment',
                'remarks'           => ''
            ];
        ...
        if ( $processData->status_id == $this->re_submit_status_id ) {
            ...
            //Send SMS
            $loginControllerInstance->SendSmsService('APP_RESUBMIT', ['{$serviceName}' => 'IPTSP License Amendment', '{$trackingNumber}' => $trackingNumber], $userMobile);
...
//            $receiverInfo = CommonFunction::getCompanyUsersEmailPhone();
        ...
        return redirect('client/iptsp-license-ammendment/list/' . Encryption::encodeId($this->process_type_id));
    }
...
//                'apps.org_per_district as org_per_pdistrict',
        ...
        if ($multi_license !== null && !empty($multi_license)) {
            $idsArray = $multi_license;
            $multi_license = AllLicense::whereIn('id', $idsArray)->pluck('name')->toArray();
            $data['appInfo']->multi_license = implode(", ", $multi_license);
        } else {
        ...
        ->toArray();
        $public_html = (string)view('REUSELicenseIssue::IPTSP.Amendment.masterView', $data);
    ...
    public function editForm( $processTypeId, $applicationId ): JsonResponse {
        ...
        $data['iptsp_license_issue'] = IPTSPLicenseAmendment::where(['id'=> $data['appInfo']['id']])->first();

        $data['nationality'] = ['' => 'Select'] + Countries::where('country_status', 'Yes')->where('nationality', '!=', '')
                ->orderby('nationality')->pluck('nationality', 'id')->toArray();

        $public_html = (string)view('REUSELicenseIssue::IPTSP.Amendment.masterEdit', $data);
        ...
        if ($data['master_data']->renew_tracking_no) {
            $data['appInfo'] = ProcessList::leftJoin( 'iptsp_license_renew as apps', 'apps.id', '=', 'process_list.ref_id' )
                ...
                ->first( [
                    ...
                    'apps.org_nm as company_name',
                    'apps.org_type as company_type',
                    'apps.reg_office_district',
                    'apps.reg_office_thana',
                    'apps.reg_office_address',
                    'apps.op_office_district',
                    'apps.op_office_thana',
                    'apps.op_office_address',
                    'apps.applicant_thana',
...
//        dd($data['appInfo']->cover_dis);
        ...
        $data['iptsp_connected_isp_info'] = IPTSPLicenseIssueConnectedISPInfo::where(['iptsp_license_issue_id'=> $data['appInfo']->ref_id])->get();
        ...
        $public_html = (string) view( 'REUSELicenseIssue::IPTSP.Amendment.search', $data );
    ...
    public function storeConnectedISPInfo($appDataId, $request) {
        if(isset($request->institution_type) && count($request->institution_type) > 0) {
            IPTSPAmendmentConnectedISPInfo::where('iptsp_license_issue_id', $appDataId)->delete();
            foreach($request->institution_type as $index => $value){
                if(empty($value)){
                    continue;
                }
                $serviceProviderObj = new IPTSPAmendmentConnectedISPInfo();
    ...
    private function storeLicenseData( $LicenseIssueObj, $request ) {
        ...
        if($request->has('op_office_address') && $request->has('op_office_thana') && $request->has('op_office_district')){
        ...
        $LicenseIssueObj->applicant_website = $request->get('applicant_website');
...
//                    //Downlink Allocation Information
    ...
    private function unfixedAmountsForGovtServiceFee( $isp_license_type, $payment_step_id, $app_id = 0 ) {
        ...
        if ($payment_step_id == 1) {
            ...
            $delay_fee = 0;
            $delay_vat_fee = 0;
            $submissionPaymentDateTime = isset($submissionPaymentData->updated_at) && $submissionPaymentData->updated_at ? date('Y-m-d', strtotime($submissionPaymentData->updated_at)) : date('Y-m-d');
