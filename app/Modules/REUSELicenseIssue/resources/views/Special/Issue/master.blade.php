...
<div class="row">
    <div class="col-md-12 col-lg-12" id="inputForm">
        ...
        <div class="card border-magenta" style="border-radius: 10px; ">
            <h4 class="card-header">Application For New Internet Service Provider (ISP) License Issue
                <a class="btn btn-sm btn-info font-weight-bold" type="button" href="{{ url('check-occupancy-availability-of-isp') }}" target="_blank">You can check vacancy availability of ISP</a>
            </h4>
            <div class="card-body">
                {!! Form::open([
...
{{--                                        <li>For New Applicant only A, B and E will be applicable.</li>--}}
                ...
                <div class="float-left">
                    <a href="{{ url('client/isp-license-issue/list/'. Encryption::encodeId(1)) }}" class="btn btn-default btn-md cancel" value="close" name="closeBtn"
...
use App\Modules\REUSELicenseIssue\Models\ISP\issue\ISPLicenseIssue;
            ...
            var company_has_application_license_type = '<?php
        echo( json_encode(ISPLicenseMaster::where('company_id',Auth::user()->working_company_id)
        ->whereNotNull('license_no')
        ->whereNull('cancellation_tracking_no')
        ->pluck('isp_license_type')->toArray()));
        ?>';

            var company_applications_div = '<?php
        echo(ISPLicenseMaster::where('company_id',Auth::user()->working_company_id)
            ->whereNotNull('license_no')
            ->where('isp_license_type',2)
        ->whereNull('cancellation_tracking_no')->count());
        ?>';
        var company_applications_dist = '<?php
        echo(ISPLicenseMaster::where('company_id',Auth::user()->working_company_id)
            ->whereNotNull('license_no')
            ->where('isp_license_type',3)
        ->whereNull('cancellation_tracking_no')->count());
        ?>';
        var company_applications_thana = '<?php
        echo(ISPLicenseMaster::where('company_id',Auth::user()->working_company_id)
            ->whereNotNull('license_no')
            ->where('isp_license_type',4)
        ->whereNull('cancellation_tracking_no')->count());
        ?>';
        ...
        form.steps({
            ...
            onStepChanging: function (event, currentIndex, newIndex) {
                ...
                if (newIndex == 1) {
                    ...
                    if($("#type_of_isp_licensese option:selected").text()=='Divisional'){
                        var license_type_text =  'বিভাগ';
                    }else if($("#type_of_isp_licensese option:selected").text()=='District'){
                        var license_type_text =  'জেলা';
                    }else if($("#type_of_isp_licensese option:selected").text()=='Thana/Upazila'){
                        var license_type_text =  'থানা/উপজেলা';
                    }

                    if(jQuery.inArray($('#type_of_isp_licensese').val(), company_has_application_license_type) !== -1){
                        alert('You are not eligible for multiple licenses of the same type.');
                        $('#type_of_isp_licensese').val('');
                        return false;
                    }else{
                        if(company_applications_div>0){
                            alert('আপনার বিভাগ ক্যাটাগরিতে ' + company_applications_div + 'টি ISP লাইসেন্স রয়েছে। ' + license_type_text + ' ক্যাটাগরির এই আবেদনটি অনুমোদনের সাথে সাথে পূর্বের লাইসেন্সসমূহ স্বয়ংক্রিয়ভাবে বাতিল হয়ে যাবে। আমি এই বিষয়ে একমত এবং আমার সজ্ঞানে জেনে বুঝে নতুন ক্যাটাগরির জন্য আবেদন করতে সম্মত হলাম।');
                        }
                        if(company_applications_dist>0){
                            alert('আপনার জেলা ক্যাটাগরিতে ' + company_applications_dist + 'টি ISP লাইসেন্স রয়েছে। ' + license_type_text + ' ক্যাটাগরির এই আবেদনটি অনুমোদনের সাথে সাথে পূর্বের লাইসেন্সসমূহ স্বয়ংক্রিয়ভাবে বাতিল হয়ে যাবে। আমি এই বিষয়ে একমত এবং আমার সজ্ঞানে জেনে বুঝে নতুন ক্যাটাগরির জন্য আবেদন করতে সম্মত হলাম।');
                        }
                        if(company_applications_thana>0){
                            alert('আপনার থানা/উপজেলা ক্যাটাগরিতে ' + company_applications_thana + 'টি ISP লাইসেন্স রয়েছে। ' + license_type_text + ' ক্যাটাগরির এই আবেদনটি অনুমোদনের সাথে সাথে পূর্বের লাইসেন্সসমূহ স্বয়ংক্রিয়ভাবে বাতিল হয়ে যাবে। আমি এই বিষয়ে একমত এবং আমার সজ্ঞানে জেনে বুঝে নতুন ক্যাটাগরির জন্য আবেদন করতে সম্মত হলাম।');
                        }
                    }
