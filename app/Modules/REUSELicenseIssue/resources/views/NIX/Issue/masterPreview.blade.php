...
//        window.opener.jQuery('#' + id + ' option:[value="' + val + '"]').replaceWith("<option value='" + val + "' selected>" + text + "</option>");
    ...
    window.opener.jQuery('#docListDiv table tbody tr').each(function (index) {
        if(jQuery(this).find('.save_file a').attr('href')) {
            var namestr = jQuery(this).find('input[type=hidden]').attr("name");
            var array = namestr.split("_");
            var lastElement = array.pop();
            const save_file = document.getElementsByClassName('saved_file_'+lastElement);
            // save_file.innerHTML = "Hello world";
            console.log(lastElement, save_file[0]);
        }
    });
    ...
    window.opener.jQuery("fieldset").css({"display": "block", "margin-top": "10px"});
    window.opener.jQuery('#application_form .card-magenta').css({ "margin-top": "10px"});
    ...
    const docElment = document.getElementById('docListDiv');
    for (const docElmentElement of docElment.getElementsByTagName('label')) {
        docElmentElement.style.marginTop = '-22px';
    }

    //Hide calender icons
    let hideCalenderIcon = document.querySelectorAll('.input-group-text');
    hideCalenderIcon.forEach(item => {
    ...
    jQuery('#previewDiv #accept_terms').attr("onclick", 'return false').prop("checked", true).css('margin-left', '-10px');
