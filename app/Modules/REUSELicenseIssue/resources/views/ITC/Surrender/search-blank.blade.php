    ...
    <div class="renewForm">
        <div class="card border-magenta">
            ...
            <div class="card-body">
                ...
                <fieldset>
                    @include('common.subviews.licenseInfo', ['mode' => 'surrender', 'url' => 'itc-license-cancellation/fetchAppData'])
                    {{-- Company Informaiton --}}
                ...
                <div class="float-left">
                    <a href="{{ url('client/itc-license-surrender/list/'. Encryption::encodeId(4)) }}" class="btn btn-default btn-md cancel" value="close" name="closeBtn"
...
<script>
    ...
    $(document).ready(function() {
        ...
        // license type part
        $('#type_of_itc_licensese').on('change', function () {
            ...
            if(this.value == 2){
                ...
                $('#district').css('display','none');
                $('#thana').css('display','none');
                addEventListenerToItcType('itc_licensese_area_division')
            }
            ...
            if(this.value == 3){
                ...
                $('#district').append('<div class="form-group row">'+
                    '{!! Form::label('itc_licensese_area_district', 'District', ['class' => 'col-md-4']) !!}'+
                    '<div class="col-md-8 {{ $errors->has('itc_licensese_area_district') ? 'has-error' : '' }}">'+
                    '{!! Form::select('itc_licensese_area_district', $districts, '', ['class' => 'form-control itc_licensese_area_district','data-type' => '3', 'id' => 'itc_licensese_area_district', 'onchange' => "getThanaByDistrictId('itc_licensese_area_district', this.value, 'itc_licensese_area_thana',0)"]) !!}'+
                    '{!! $errors->first('itc_licensese_area_district', '<span class="help-block">:message</span>') !!}'+
                    '</div>'+
                    '</div>');
                $('#thana').css('display','none');
                addEventListenerToItcType('itc_licensese_area_district')
            }
            ...
            if(this.value == 4){
                ...
                $('#district').append('<div class="form-group row">'+
                    '{!! Form::label('itc_licensese_area_district', 'District', ['class' => 'col-md-4']) !!}'+
                    '<div class="col-md-8 {{ $errors->has('itc_licensese_area_district') ? 'has-error' : '' }}">'+
                    '{!! Form::select('itc_licensese_area_district', [''=>'Select'], '', ['class' => 'form-control itc_licensese_area_district', 'id' => 'itc_licensese_area_district', 'onchange' => "getThanaByDistrictId('itc_licensese_area_district', this.value, 'itc_licensese_area_thana',0)"]) !!}'+
                    '{!! $errors->first('itc_licensese_area_district', '<span class="help-block">:message</span>') !!}'+
                    '</div>'+
                    '</div>');
                $('#thana').css('display','inline');

                addEventListenerToItcType('itc_licensese_area_thana')

            }
            getHelpText();
        });
        $(document).on('change','#itc_licensese_area_division' ,function (){
            $("#itc_licensese_area_thana").prop('selectedIndex',0);
        });
        ...
        // payment panel part
        $("#type_of_itc_licensese").change(function() {
            let oss_fee = 0;
            let vat = 0;
            $.ajax({
                type: "POST",
                url: "{{ url('itc-license-issue/get-payment-data-by-license-type') }}",
                data: {
    ...
    const itcLicenseAreaListeners = {
        'itc_licensese_area_division' : 0,
        'itc_licensese_area_district' : 0,
        'itc_licensese_area_thana' : 0,
    };
    function addEventListenerToItcType(selector) {
        const element = document.getElementById(selector);
        if(!itcLicenseAreaListeners[selector]){
            element.addEventListener("change", function(event) {
                const itcLicenseType = document.getElementById('type_of_itc_licensese').value;
                const currentItcLicenseType = event.target.dataset.type;
                const ITC_LICENSE_TYPES = {
                    2: 'itc_licensese_area_division',
                    3: 'itc_licensese_area_district',
                    4: 'itc_licensese_area_thana',
                };
                let areaId =  ITC_LICENSE_TYPES[itcLicenseType] ? document.getElementById(ITC_LICENSE_TYPES[itcLicenseType]).value : '';

                if(!areaId || (itcLicenseType != currentItcLicenseType)){
                    return
                }
                $.ajax({
                    type: "POST",
                    url: "{{ url('itc-license-issue/check-application-limit') }}",
                    data: {itcLicenseType,areaId},
                    success: function(response) {
                        // error
            ...
            itcLicenseAreaListeners[selector] = 1;
        }
