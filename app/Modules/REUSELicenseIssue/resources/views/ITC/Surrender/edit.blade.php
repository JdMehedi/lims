        ...
        <div class="col-md-12 col-lg-12" id="inputForm">
            ...
            <div class="card border border-magenta" style="border-radius: 10px; ">
                <div class="card-header" style="display: flex; justify-content: space-between;">
                    <h4 style="border-bottom-width: 0px;">Application For ITC License Surrender</h4>
...
{{--                        </div>--}}
                    ...
                    <div class="float-left">
                        <a href="{{ url('client/itc-license-ammendment/list/'. Encryption::encodeId(3)) }}"
...
<script>
    $(document).ready(function() {
        ...
        $("#itc_type").click(function () {
            if(this.checked)
            {
                $("#itc_type_options select").removeClass('input_disabled')
            }else{
                $("#itc_type_options select").addClass('input_disabled')
            }
        });
        ...
        $('.add_bandwidth_row').click(function(){
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('itc-license-renew/add-bandwidth-row') }}",
                data: {
                    lastRowId: lastRowId,
                },
                success: function(response) {
...
{!! Form::label('proposal_employee_${lastRowId+1}', 'Proposed of Employee', ['class' => 'col-md-4']) !!}
    ...
    <div class="row">
        ...
        $('.add_connectivity_row').click(function(){
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('itc-license-renew/add-connectivity-row') }}",
                data: {
                    lastRowId: lastRowId,
                },
                success: function(response) {
