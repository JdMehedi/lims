...
<div class="row">
    <div class="renewForm">
        ...
        <h4 class="card-header">Application for Tower Sharing License Surrender</h4>
            <div class="card-body">
                {!! Form::open([
                ...
                <fieldset>
                    @include('common.subviews.licenseInfo', ['mode' => 'surrender', 'url' => 'tc-license-cancellation/fetchAppData'])
                    {{-- Company Informaiton --}}
                ...
                <div class="float-left">
                    <a href="{{ url('client/tc-license-cancellation/list/'. Encryption::encodeId(69)) }}" class="btn btn-default btn-md cancel" value="close" name="closeBtn"
...
<script>
    if (typeof isAppLimitExceeded === 'undefined') {
    ...
    $(document).ready(function() {
        ...
        $('.finish').on('click', function (e) {
            {{--popupWindow = window.open('<?php echo URL::to('/tc-license-issue/preview'); ?>', 'Sample', '');--}}
            openPreviewV2();
        });
        ...
        $("#type_of_isp_licensese").change(function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('tc-license-issue/get-payment-data-by-license-type') }}",
                data: {
...
<div class="row form-group">
    ...
    if (typeof ispLicenseAreaListeners === 'undefined') {
    ...
    function addEventListenerToIspType(selector) {
        ...
        if(!ispLicenseAreaListeners[selector]){
            element.addEventListener("change", function(event) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('tc-license-issue/check-application-limit') }}",
