...
<div class="row">
    <div class="renewForm">
        ...
        <h4 class="card-header">Application for MNO License Surrender</h4>
            <div class="card-body">
                {!! Form::open([
                ...
                <fieldset>
                    @include('common.subviews.licenseInfo', ['mode' => 'surrender', 'url' => 'mno-license-cancellation/fetchAppData'])
                    {{-- Company Informaiton --}}
                ...
                <div class="float-left">
                    <a href="{{ url('client/mno-license-cancellation/list/'. Encryption::encodeId(61)) }}" class="btn btn-default btn-md cancel" value="close" name="closeBtn"
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.finish').on('click', function (e) {
            {{--popupWindow = window.open('<?php echo URL::to('/mno-license-issue/preview'); ?>', 'Sample', '');--}}
            openPreviewV2();
        });
        ...
        $("#type_of_isp_licensese").change(function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('mno-license-issue/get-payment-data-by-license-type') }}",
                data: {
    ...
    function addEventListenerToIspType(selector) {
        ...
        if(!ispLicenseAreaListeners[selector]){
            element.addEventListener("change", function(event) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('mno-license-issue/check-application-limit') }}",
