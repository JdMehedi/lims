...
@section('content')
    ...
    <div class="application-btn-group">
        <?php
            ...
            //dump($group_wise_services, $biddingLicense);
            $index = 0;
        ...
        @if (ACL::getAccsessRight($service->acl_name, '-A-') )
                <a @if($issue_warning_bool == 0) href="{{ URL::to("special_service/add/" . \App\Libraries\Encryption::encodeId($service->id)) }}" @endif
                       class="{{( ($service->license_type == 'bidding_license') && ($index == -1) && ($showModuleId != $service->id)) || ($service->status!=1) ? 'disabled disable-button-opacity' : ''}}">
                    {!! Form::button('<b>'.ucfirst(last(explode('-', $service->type_key))).'</b>', [
                        'type' => 'button',
                        'class' => 'btn btn-success new_application', 'id' => 'modelButton'
                    ]) !!}
                </a>
        @endif
        ...
        @if($user_type != '5x505')
            ...
            class="btn btn-success {{ ($service->status!=1) ? 'disabled disable-button-opacity' : ''}}" style="border-radius: 10px;">
