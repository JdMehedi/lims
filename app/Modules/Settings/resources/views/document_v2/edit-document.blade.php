...
<div class="modal-body">
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="form-group col-md-12 row">
                ...
                <div class="col-md-8">
                    {!! Form::text('name', $document_info->name, ['class'=>'form-control required input-sm', 'id' => 'name', 'placeholder'=>'Document name']) !!}
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-lg-11">
            <div class="form-group col-md-12 row">
                {!! Form::label('simple_file','Simple File',['class'=>'col-md-3  required-star']) !!}
                <div class="col-md-9">
                    <div class="input-group">
                        {!! Form::file('simple_file', ['class' => 'form-control input-sm', 'id' => 'simple_file']) !!}
                        <input type="hidden" name="preview_simple_file"
                               value="{{ $document_info->simple_file ? asset($document_info->simple_file) : '' }}">
                    </div>
                    @if($document_info->simple_file)
                        <a class="m-2 btn btn-warning btn-sm" href="{{ asset($document_info->simple_file) }}"
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="form-group col-md-12 row">
                {!! Form::label('company type','Company Type',['class'=>'text-left required-star col-md-4']) !!}
                <div class="col-md-8">
                    <label class="radio-inline">
                        {!! Form::radio('company_doc_type', 2, ($document_info->company_doc_type == '2' ? true : false), ['class'=>'required', 'id' => 'doc_n_company_type_proprietorship']) !!}
                        Proprietorship
                    </label>
                    <label class="radio-inline">
                        {!! Form::radio('company_doc_type', 1, ($document_info->company_doc_type == '1' ? true : false), ['class'=>'required', 'id' => 'doc_n_company_type_others']) !!}
                        Others
                    </label>
                    <label class="radio-inline">
                        {!! Form::radio('company_doc_type', 3, ($document_info->company_doc_type == '3' ? true : false), ['class'=>'required', 'id' => 'doc_n_company_type_both']) !!}
                        Both
                    </label>
                </div>
            </div>
...
<script>
    $(document).ready(function () {
        ...
        function formSubmit() {
            ...
            const simpleFileInput = $("#simple_file")[0];
            let formData = new FormData();
            formData.append('name', $("#name").val());
            formData.append('min_size', $("#min_size").val());
            formData.append('max_size', $("#max_size").val());
            formData.append("simple_file", simpleFileInput.files[0]);
            formData.append("status", $(':radio[name=status]:checked').val());
            formData.append("company_doc_type", $(':radio[name=company_doc_type]:checked').val());
