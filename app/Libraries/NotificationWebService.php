...
//            return json_encode($data);
     ...
     * @return array
     */
    public function sendEmailOld(array $email_data)
    {
    ...
    {
        $config = [
            'host' => config('app.SMTP_EMAIL_HOST'),
            'port' => config('app.SMTP_EMAIL_PORT'),
            'username' => config('app.SMTP_EMAIL_USERNAME'),
            'password' => config('app.SMTP_EMAIL_PASSWORD'),
            'from' => config('app.SMTP_EMAIL_FROM'),
            'name' => config('app.SMTP_EMAIL_NAME'),
            'secure' => config('app.SMTP_EMAIL_SECURE'),
        ];
        ...
        $phpmailer->setFrom($config['from'], $config['name']);
        $phpmailer->Subject = $email_data['subject'];
        $phpmailer->Body = $email_data['bodyHtml'];
        $phpmailer->addAddress($email_data['recipient']);

        if (isset($email_data['cc'])) {
            $phpmailer->addCC($email_data['cc']);
        }

        if (isset($email_data['bcc'])) {
            $phpmailer->addBCC($email_data['bcc']);
        }

        try {
            $msg = 'Send Email Success.';
            $phpmailer->send();
            return ['status' => 1, 'msg' => $msg, 'message_id' => $phpmailer->getLastMessageID()];
        } catch (\Exception $exception) {  
            return ['status' => 0, 'msg' => $exception->getMessage() . $exception->getFile() . $exception->getLine()];
        }
    }
